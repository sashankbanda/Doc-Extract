...
# Keys to explicitly ignore/drop (noise from headers/footers)
BLACKLISTED_KEYS = {
    "section", "page", "continued", "form", "part", "table", "summary", 
    "total", "subtotal", "date", "time", "run date", "run time",
    "report", "generated by", "prepared by"
}
...
def find_canonical_name(source_key: str) -> Optional[str]:
    ...
    # Try One-Way Containment (Synonym IN Key)
    # Ex: Key="Insured Name (DBA)" -> Contains "Insured Name" -> Match
    # DO NOT allow Key IN Synonym (e.g. Key="Name" in Synonym="Insured Name" -> Match) as it is too broad.
...
class LLMService:
    ...
    async def structure_document(
        ...
        for item in raw_items:
            ...
            if source_key and source_key.strip().lower() in BLACKLISTED_KEYS:
                 # Skip purely noise keys
                 continue
